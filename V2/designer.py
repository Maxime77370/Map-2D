...
class Game:
    ...
    def __init__(self):
        ...
        self.tilemap = Tilemap(self.tileset, size=(16, 32), resize = 2)
        del self.tileset
        self.tilemap.set_reset()

        self.menuset = Tileset(file) 
        self.menumap = Tilemap(self.menuset, resize = 2)
        del self.menuset
...
class Tilemap:
    ...
    def set_generator(self, seed = np.random.randint(0,10000)):
        ...
        for x in range(self.sprite_size[1]):
            for y in range(255-62, self.sprite_size[0]):
